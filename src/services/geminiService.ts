...
export const getTrackSuggestions = async (): Promise<Track[]> => {
  ...
  try {
    ...
    const tracksList = allTracks.map(track =>
      `${track.title} by ${track.author} (${track.category})`
    ).join('\n');
...
Return only the JSON array, no other text.`;
    ...
    try {
      ...
      console.warn('Failed to parse AI response as JSON, falling back to text parsing:', parseError);
       suggestedTitles = text.split(',').map(title => title.trim().replace(/["[\]]/g, ''));
    }
    ...
    for (const title of suggestedTitles) {
     ...
     while (suggestions.length < 3) {
      const remaining = allTracks.filter(t => !suggestions.find(s => s.id === t.id));
